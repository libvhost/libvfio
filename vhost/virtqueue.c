...
void vhost_vq_init(VirtQueue *vq) {
  ...
  align_size = vring_size(vq->size, VIRTIO_PCI_VRING_ALIGN);
  q_mem = vhost_malloc(align_size);
...
void virtring_add(VirtQueue *vq, struct iovec *iovec, int num_out, int num_in,
  ...
  for (i = 0; i < num_out; ++i) {
    ...
    DEBUG("    item %2d addr %p len 0x%-10" PRIx64 " flags %s\n", n,
          iovec[i].iov_base, iovec[i].iov_len, flags_name);
    last_n = n;
    n = desc[n].next;
  }
...
static inline bool more_used(const VirtQueue *vq) {
  rmb();
  // NOTICE: the shared used->idx range is [0, 2^16 -1], not [0, ring_num - 1];
  return vq->last_used_idx != vq->vring.used->idx;
}

static void reset_desc(VirtQueue *vq, uint16_t head) {
  uint16_t id = head;
  // reset the desc;
...
int virtqueue_get(VirtQueue *vq, VhostTask **out_task) {
  ...
  DEBUG("[VIRTIO] USED RING last_used_idx: %d, last_used: %d, req id: %d len: "
        "%d task: %p used: %d\n",
