...
typedef struct NVME_PACKED NvmeBar {
  uint64_t cap;
  uint32_t vs;
  uint32_t intms;
  uint32_t intmc;
...
#define NVME_CMBMSMC_SET_CRE                                                   \
  (cmbmsc, val)(cmbmsc |= (uint64_t)(val & CMBLOC_OFST_MASK)                   \
                          << CMBMSC_CRE_SHIFT)
...
#define NVME_PMRMSC_CBA(pmrmsc) ((pmrmsc >> PMRMSC_CBA_SHIFT) & PMRMSC_CBA_MASK)
...
typedef struct NVME_PACKED NvmeSglDescriptor {
  uint64_t addr;
  uint32_t len;
  uint8_t rsvd[3];
  uint8_t type;
...
typedef struct NVME_PACKED NvmeCmd {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t nsid;
...
typedef struct NVME_PACKED NvmeDeleteQ {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t rsvd1[9];
...
typedef struct NVME_PACKED NvmeCreateCq {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t rsvd1[5];
...
typedef struct NVME_PACKED NvmeCreateSq {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t rsvd1[5];
...
typedef struct NVME_PACKED NvmeIdentify {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t nsid;
...
typedef struct NVME_PACKED NvmeRwCmd {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t nsid;
...
typedef struct NVME_PACKED NvmeDsmCmd {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t nsid;
...
typedef struct NVME_PACKED NvmeDsmRange {
  uint32_t cattr;
  uint32_t nlb;
  uint64_t slba;
} NvmeDsmRange;
...
typedef struct NVME_PACKED NvmeCopyCmd {
  uint8_t opcode;
  uint8_t flags;
  uint16_t cid;
  uint32_t nsid;
...
typedef struct NVME_PACKED NvmeCopySourceRange {
  uint8_t rsvd0[8];
  uint64_t slba;
  uint16_t nlb;
  uint8_t rsvd18[6];
...
typedef struct NVME_PACKED NvmeAerResult {
  uint8_t event_type;
  uint8_t event_info;
  uint8_t log_page;
  uint8_t resv;
...
typedef struct NVME_PACKED NvmeZonedResult {
  uint64_t slba;
} NvmeZonedResult;

typedef struct NVME_PACKED NvmeCqe {
  uint32_t result;
  uint32_t dw1;
  uint16_t sq_head;
  uint16_t sq_id;
...
typedef struct NVME_PACKED NvmeFwSlotInfoLog {
  uint8_t afi;
  uint8_t reserved1[7];
  uint8_t frs1[8];
  uint8_t frs2[8];
...
typedef struct NVME_PACKED NvmeErrorLog {
...
typedef struct NVME_PACKED NvmeSmartLog {
...
typedef struct NVME_PACKED NvmePSD {
  uint16_t mp;
  uint16_t reserved;
  uint32_t enlat;
  uint32_t exlat;
...
typedef struct NVME_PACKED NvmeIdCtrl {
  uint16_t vid;
  uint16_t ssvid;
  uint8_t sn[20];
  uint8_t mn[40];
...
typedef struct NVME_PACKED NvmeRangeType {
  uint8_t type;
  uint8_t attributes;
  uint8_t rsvd2[14];
  uint64_t slba;
...
typedef struct NVME_PACKED NvmeLBAF {
  uint16_t ms;
  uint8_t ds;
  uint8_t rp;
} NvmeLBAF;

typedef struct NVME_PACKED NvmeLBAFE {
  uint64_t zsze;
  uint8_t zdes;
  uint8_t rsvd9[7];
} NvmeLBAFE;
...
typedef struct NVME_PACKED NvmeIdNs {
  uint64_t nsze;
  uint64_t ncap;
  uint64_t nuse;
  uint8_t nsfeat;
...
typedef struct NVME_PACKED NvmeIdNsDescr {
  uint8_t nidt;
  uint8_t nidl;
  uint8_t rsvd2[2];
} NvmeIdNsDescr;
...
typedef struct NVME_PACKED NvmeIdNsZoned {
  uint16_t zoc;
  uint16_t ozcs;
  uint32_t mar;
  uint32_t mor;
...
typedef struct NVME_PACKED NvmeZoneReportHeader {
  uint64_t nr_zones;
  uint8_t rsvd[56];
...
typedef struct NVME_PACKED NvmeZoneDescr {
  uint8_t zt;
  uint8_t zs;
  uint8_t za;
  uint8_t rsvd3[5];
...
#define NVME_BUILD_BUG_ON(eq) static_assert(eq, #eq)

NVME_BUILD_BUG_ON(sizeof(NvmeBar) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeAerResult) == 4);
NVME_BUILD_BUG_ON(sizeof(NvmeZonedResult) == 8);
NVME_BUILD_BUG_ON(sizeof(NvmeCqe) == 16);
NVME_BUILD_BUG_ON(sizeof(NvmeDsmRange) == 16);
NVME_BUILD_BUG_ON(sizeof(NvmeCopySourceRange) == 32);
NVME_BUILD_BUG_ON(sizeof(NvmeCmd) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeDeleteQ) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeCreateCq) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeCreateSq) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeIdentify) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeRwCmd) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeDsmCmd) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeCopyCmd) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeRangeType) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeErrorLog) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeFwSlotInfoLog) == 512);
NVME_BUILD_BUG_ON(sizeof(NvmeSmartLog) == 512);
NVME_BUILD_BUG_ON(sizeof(NvmeEffectsLog) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeIdCtrl) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeIdCtrlZoned) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeIdCtrlNvm) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeLBAF) == 4);
NVME_BUILD_BUG_ON(sizeof(NvmeLBAFE) == 16);
NVME_BUILD_BUG_ON(sizeof(NvmeIdNs) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeIdNsZoned) == 4096);
NVME_BUILD_BUG_ON(sizeof(NvmeSglDescriptor) == 16);
NVME_BUILD_BUG_ON(sizeof(NvmeIdNsDescr) == 4);
NVME_BUILD_BUG_ON(sizeof(NvmeZoneDescr) == 64);
NVME_BUILD_BUG_ON(sizeof(NvmeDifTuple) == 8);
